...
uint shmem[4];  // shared memory pages
uint shmemusers[4];  // num of shmem users
...
int
allocuvm(pde_t *pgdir, uint oldsz, uint newsz, uint shmem_bound)
{
  char *mem;
  uint a;

  if(newsz > shmem_bound)
    return 0;
  if(newsz < oldsz)
    return oldsz;
...
// of it for a child.
pde_t*
copyuvm(pde_t *pgdir, uint sz, uint shmem_bound)
{
  pde_t *d;
  pte_t *pte;
  uint pa, i;
  ...
  for(i = shmem_bound; i < USERTOP; i += PGSIZE){
    if((pte = walkpgdir(pgdir, (void*)i, 0)) == 0)
      panic("copyuvm: pte should exist");
    if(!(*pte & PTE_P))
      panic("copyuvm: page not present");
...
uint shmemget(uint x)
{
  ...
  else if(proc->shmem[x] != 0){
    return proc->shmem[x];
}
  else{
    addr = proc->shmem_bound - PGSIZE;
    if(addr < proc->sz)
      return 0;
    mappages(proc->pgdir, (char*)addr, PGSIZE, shmem[x], PTE_W|PTE_U|PTE_S);
    proc->shmem[x] = addr;
    proc->shmem_bound = addr;
    shmemusers[x]++;
    return addr;
  }
}
