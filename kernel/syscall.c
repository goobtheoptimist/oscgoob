...
{
  if(!((addr >= PGSIZE && addr + 4 <= p->sz)
    || (addr >= p->shmem_bound && addr + 4 <= USERTOP)))
    return -1;
  *ip = *(int*)(addr);
  return 0;
}
...
fetchstr(struct proc *p, uint addr, char **pp)
{
  ...
  else if(addr >= p->shmem_bound && addr < USERTOP){
    ep = (char*)USERTOP;
}
  else
    return -1;
...
argptr(int n, char **pp, int size)
{
  ...
  if(argint(n, &i) < 0)
    return -1;
  if(!(((uint)i >= PGSIZE && (uint)i + size <= proc->sz) || ((uint)i >= proc->shmem_bound && (uint)i + size <= USERTOP)))
    return -1;
  *pp = (char*)i;
  return 0;
}
